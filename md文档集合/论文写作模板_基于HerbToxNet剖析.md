...
### 2.3 Related Work（相关工作）— 800-1200字

#### HerbToxNet 写法剖析

Zhu2026 的 Related Work 分为 **4个子节**，每个子节末尾都有**批判性总结**：

**模式识别：每个子节的组织方式**

| 子节 | 字数 | 结构 |
|------|------|------|
| 2.1 QSAR-based methods | ~150 | 方法简介 → 代表性工作(3-4个引用) → 局限性(最后1-2句) |
| 2.2 Network toxicology | ~150 | 同上 |
| 2.3 Herb property theory | ~150 | 同上 |
| 2.4 Multi-label + HAN | ~200 | 方法简介 → 与本文的关系 → 本文如何不同 |

**关键写法特征：**
- 每个子节最后1-2句都是 **"局限性陈述"**，为本文做铺垫
  - QSAR: "...restricted by the limited generalization ability"
  - Network: "...still rely on incomplete... data, leading to biased predictions"
  - Herb property: "...qualitative and subjective nature... results in ambiguous data"
- 引用密度极高（每2-3句出现一组引用）
- 对最接近的工作不加贬损，而是客观陈述差异

#### 模板要求

```
【字数】800-1200英文词
【子节数量】3-4个，按方法论维度或技术路线分类
...
【内容要求】
  □ 至少引用15-25篇相关工作
  □ 每个引用都要有其存在必要性（不能为了凑数）
  □ 分类标准要清晰互斥（如按技术路线分：QSAR / 网络 / 传统理论）
  □ 每个子节末尾必须指出该方法的局限性
  □ 最后一个子节应是最接近本文的工作，要明确说明本文与它的差异
  □ 不贬低他人工作——用客观词汇："limited by" / "restricted to" / "often fails to"

【禁止】
  — 在此章详细描述自己的方法
  — 使用"no existing method can..."等绝对化表述
  — 把相关工作写成流水账（Name1 did X, Name2 did Y, Name3 did Z...）
    正确做法：按主题归类→每个主题挑代表性工作→指出共性局限
```
...
这是论文最核心的章节，Zhu2026 的组织方式展现出明显的**工程方法论风格**：

**章节结构：**
```
3. The proposed methodology
  ├── 章节导言段（1段，概述4个模块+引用Fig.1）
  ├── 3.1 Heterogeneous graph construction（~300字）
  ├── 3.2 Herb representation learning by HAN（~400字）
  ├── 3.3 Contrastive learning with dynamic coefficient（~500字）
  └── 3.4 Toxicity prediction based on weighted label fusion（~400字）
```

**每个子节（3.1-3.4）的内部结构分析：**

以 3.3（Contrastive learning）为例：

| 段落功能 | 内容 | 字数 |
|---------|------|------|
| 动机句 | 为什么需要这个模块 | ~50字 |
| 现有方法简述 | 标准对比学习做了什么 | ~80字 |
| 问题指出 | 多标签场景下标准方法为何不够 | ~100字 |
| 本文方案 | 我们的动态系数如何解决 | ~150字 |
| 公式推导 | 从 Eq.(9) 到 Eq.(13) | 公式为主 |
| 关键设计说明 | 超参数t的作用 | ~80字 |

**公式使用规范（重要！）：**
- 每个公式 = 先在正文中解释含义 → 再出现公式 → 再解释每个符号的含义
- 格式：定义（自然语言）→ 公式编号(1) → 符号说明（"where X denotes..., Y represents..."）
- 共约20个公式，编号连续
- 重要的公式正文要"说人话"解释直觉含义

**Algorithm 伪代码（Algorithm 1）：**
- 位置：方法论章末尾、实验章之前
- 格式：Input → Output → 分阶段步骤（⊳标注4个阶段）
- 行号标记，方便实验部分引用
- 约30行伪代码
...
```
【字数】2000-3000英文词（不含公式）
【子节数量】3-5个模块，每个模块一个小节

【章节导言段（强制，~100字）】：
  □ "The framework of [方法名] is illustrated in Fig. 1."
  □ "It comprises N main modules: (i) ... (ii) ... (iii) ..."
  □ "These modules are organized sequentially, enabling [方法名] to..."
  □ 最后一句说明整体如何工作
...
【公式规范】（强制）：
  ...
  □ 向量/矩阵用小写/大写粗体（𝐱, 𝐖）
  □ 集合用花体（, ）
  □ 使用"where"从句逐项说明（不是"in which"）
  □ 重要公式前必须有自然语言直觉解释
  □ 不要公式堆砌——公式之间要有过渡文字

【图和表】：
  □ Fig.1: 框架总览图（方法论章开头，必须）
  □ 表格：符号表（Notation table）可选但推荐
  □ Algorithm伪代码：1个，在方法论末尾
